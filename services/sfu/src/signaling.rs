...
pub async fn join_room(
    ...
    // Set waiting room status: Guests enter waiting room by default in meeting mode
    if role == ParticipantRole::Guest && mode == RoomMode::Meeting {
    ...
    info!(
        ...
        body.participant_id, 
        role, 
        participant.info.is_in_waiting_room
    );
    ...
    let response = JoinRoomResponse {
        success: true,
        participant_id: body.participant_id,
        is_in_waiting_room: participant.info.is_in_waiting_room,
...
pub async fn admit_participant(
    ...
    let result = room.participants.alter(&body.participant_id, |_, mut p| {
