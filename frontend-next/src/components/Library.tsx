...
import {
    ...
    Video, Mic, Upload, Monitor, Radio, Users, Folder, ChevronRight, ChevronDown
} from 'lucide-react';
...
export const Library: React.FC<LibraryProps> = ({ onEditAsset }) => {
    ...
    const loadAssets = useCallback(async () => {
        ...
        setIsLoading(true);
        try {
            const { assets: fetchedAssets } = await ApiClient.listAssets(user.id);
    ...
    const groupedAssets = useMemo(() => {
        ...
        if (!groupByProject) return [{ id: 'all', name: 'All Assets', assets: filtered }];

        // Group by room_id
        const groups: Record<string, { id: string, name: string, assets: ExtendedAsset[] }> = {};
        filtered.forEach(asset => {
            const gid = asset.room_id || 'uncategorized';
            const gname = asset.room_name || 'General Uploads';
            if (!groups[gid]) groups[gid] = { id: gid, name: gname, assets: [] };
            groups[gid].assets.push(asset);
        });

        return Object.values(groups).sort((a, b) => a.name.localeCompare(b.name));
    }, [assets, searchQuery, activeFilter, sortBy, sortDesc, archivedIds, groupByProject]);
    ...
    const handleDelete = async () => {
        if (deleteId) {
            ...
            try {
                await ApiClient.deleteAsset(deleteId, asset.origin === 'upload' ? 'upload' : 'recording');
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center bg-app-surface border border-app-border rounded-lg p-1 shadow-sm">
                        <button
                            ...
                            className={`p-1.5 rounded-md transition-colors flex items-center gap-1.5 px-2 ${groupByProject ? 'bg-indigo-500/10 text-indigo-500' : 'text-content-medium hover:text-content-high hover:bg-app-bg'}`}
            ...
            <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
                ...
                {isLoading ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {[1, 2, 3, 4].map(i => <div key={i} className="h-[280px] bg-app-surface rounded-xl animate-pulse" />)}
                    </div>
                ) : groupedAssets.length === 0 || (groupedAssets.length === 1 && groupedAssets[0].assets.length === 0) ? (
                        ...
                        {groupedAssets.map(group => (
                            <div key={group.id} className="flex flex-col">
                                {groupByProject && (
                                    ...
                                    >
                                        <div className="w-8 h-8 bg-indigo-500/10 rounded-lg flex items-center justify-center text-indigo-500 border border-indigo-500/20 group-hover:bg-indigo-500/20 transition-colors">
                                            <Folder className="w-4 h-4" />
                                        </div>
                                        <h2 className="text-sm font-bold text-content-high uppercase tracking-wider flex-1">{group.name} <span className="ml-2 text-content-low font-normal">({group.assets.length})</span></h2>
                                        {expandedGroups.has(group.id) ? <ChevronDown className="w-4 h-4 text-content-low" /> : <ChevronRight className="w-4 h-4 text-content-low" />}
                                    </div>
                                )}

                                {(!groupByProject || expandedGroups.has(group.id) || expandedGroups.size === 0) && (
                                    viewMode === 'grid' ? (
                                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
