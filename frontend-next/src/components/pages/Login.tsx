...
export const Login: React.FC<LoginProps> = ({ onSuccess, onNavigateBack, onNavigateToSignup }) => {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Auth context will update and App will auto-redirect to dashboard
            if (onSuccess) {
                onSuccess();
            }
        } catch (err: any) {
        ...
        <div className="min-h-screen bg-app-bg flex items-center justify-center relative overflow-hidden font-sans">
            ...
            <div className="w-full max-w-md bg-app-surface border border-app-border rounded-3xl p-8 md:p-10 shadow-2xl relative z-10 animate-scale-in">
                ...
                <div className="mt-8 pt-8 border-t border-app-border text-center">
                    <button onClick={onNavigateToSignup} className="text-sm text-content-medium hover:text-content-high transition-colors">
                        Don't have an account? <span className="text-app-accent font-medium">Sign up</span>
                    </button>
                    {onNavigateBack && (
                        <div className="mt-4">
                            <button onClick={onNavigateBack} className="text-xs text-content-low hover:text-content-medium">
                                &#8592; Back to home
                            </button>
                        </div>
                    )}
