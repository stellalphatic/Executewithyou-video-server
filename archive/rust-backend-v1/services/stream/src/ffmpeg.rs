...
impl FfmpegManager {
    ...
    pub async fn start_rtp_ingest(
        ...
        let sdp_path = self.config.hls_output_dir.join(format!("{}.sdp", room_id));
        tokio::fs::write(&sdp_path, sdp_content).await?;
        ...
        cmd.args([
            ...
            "-i", sdp_path.to_str().unwrap(),
            // Video encoding (transcode VP8 to H264 for HLS)
            "-c:v", "libx264",
            "-preset", "veryfast",
            "-tune", "zerolatency",
            "-b:v", &format!("{}k", self.config.max_bitrate),
            ...
            "-bufsize", &format!("{}k", self.config.max_bitrate * 2),
            "-g", "60",
             // Audio encoding (assume silence or handle audio later - for now just generate silence if missing or copy)
             // Note: SDP above only has video. If we want audio we need another m-line.
             // For POC, let's just do video.
             "-an", // Disable audio for now to prevent failures if no audio track
            // HLS output
            "-f", "hls",
            "-hls_time", &self.config.segment_duration.to_string(),
            "-hls_list_size", &self.config.playlist_size.to_string(),
        ...
        info!("Started RTP ingest for room {} on port {} (pid: {})", room_id, port, pid);
